...
impl<Si, F, Item> Sink<Item> for SinkMapErr<Si, F>
where
    Si: Sink<Item>,
    F: FnOnce1<Si::Error>,
{
    ...
    fn poll_ready(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
        self.as_mut().project().sink.poll_ready(cx).map_err(|e| self.as_mut().take_f().call_once(e))
    }
    ...
    fn start_send(mut self: Pin<&mut Self>, item: Item) -> Result<(), Self::Error> {
        self.as_mut()
            ...
            .sink
            .start_send(item)
            .map_err(|e| self.as_mut().take_f().call_once(e))
    }

    fn poll_flush(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
        self.as_mut().project().sink.poll_flush(cx).map_err(|e| self.as_mut().take_f().call_once(e))
    }

    fn poll_close(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
        self.as_mut().project().sink.poll_close(cx).map_err(|e| self.as_mut().take_f().call_once(e))
    }
}
